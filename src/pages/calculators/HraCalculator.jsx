      ...
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-xl overflow-hidden grid md:grid-cols-2 border border-gray-200">
        ...
        <div className="p-8 bg-white">
          ...
          <div className="space-y-4">
            ...
            {/* HRA Received */}
            <div>
              <label className="block text-gray-700 text-sm font-medium">HRA Received (₹)</label>
              <input
                type="number"
            ...
            {/* Rent Paid */}
            <div>
              <label className="block text-gray-700 text-sm font-medium">Rent Paid (₹)</label>
              <input
                type="number"
        ...
        <div className="p-8 bg-linear-to-br from-cyan-600 to-blue-700 text-white flex flex-col justify-center">
          ...
          {result ? (
            <div className="bg-white/20 rounded-xl p-4 shadow-lg backdrop-blur-md space-y-2">
              <p className="text-lg">
                <strong>Exempt HRA:</strong> ₹{Number(result.hraExemptMonthly).toLocaleString()}
              </p>
              <p className="text-lg">
                <strong>Taxable HRA:</strong> ₹{Number(result.hraTaxableMonthly).toLocaleString()}
              </p>

              <p className="text-sm mt-3">
                <strong>Exempt (Yearly):</strong> ₹{Number(result.hraExemptYearly).toLocaleString()}
              </p>
              <p className="text-sm">
                <strong>Taxable (Yearly):</strong> ₹{Number(result.hraTaxableYearly).toLocaleString()}
              </p>
            </div>
          ) : (
