...
const [activeInfo, setActiveInfo] = useState(null); 
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          <span className="px-2 py-1 rounded-full text-xs bg-amber-50 text-amber-700 border border-amber-100">{fund.risk}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-6 justify-between">
                    <div>
                      <div className=" items-end gap-3">
                        <h2 className="text-4xl font-extrabold text-emerald-600">+31.29% <span className="text-gray-400 text-sm font-medium">3Y annualized</span></h2>
                    ...
                    <div className="flex gap-3 items-center">
                      <button onClick={openBuy} className="px-5 py-2 rounded-xl bg-emerald-600 text-white font-semibold shadow hover:bg-emerald-700">Invest Now</button>
      
                      <div className="text-right text-sm text-slate-500">
                        <div>Nav: <span className="text-slate-900 font-medium">{fund.nav}</span></div>
                        <div>Fund size: <span className="text-slate-900 font-medium">{fund.fundSize}</span></div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="bg-gradient-to-r from-amber-50 to-white p-3 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-10 h-10 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold">{4.5}</div>
  ...
  <div className="p-4 bg-orange-50 rounded-xl shadow-sm text-center">
    ...
    <p className="font-semibold">₹{fund.minSip}</p>
          ...
          <h4 className="text-lg font-semibold text-slate-900 mb-3">Advanced Ratios</h4>
  ...
  <div className="bg-white shadow-lg rounded-2xl p-6 flex-1">
    ...
    <div className="flex gap-4 mb-6">
      <button
        ...
        className={`px-4 py-2 rounded-full border font-medium ${
          mode === "sip" ? "bg-green-600 text-white border-green-600" : "bg-white border-gray-300"
        }`}
      >
        Monthly SIP
      </button>
      ...
      <button
        ...
        className={`px-4 py-2 rounded-full border font-medium ${
          mode === "lumpsum" ? "bg-green-600 text-white border-green-600" : "bg-white border-gray-300"
        }`}
      >
        One-Time
      </button>
    ...
    {mode === "sip" && (
      <div>
        <p className="font-semibold text-lg mb-2">₹{sipAmt.toLocaleString()} per month</p>
        <input
          type="range"
          min={fund.minSip}
          max={50000}
        ...
        <p className="text-gray-600 mb-2">Duration: {duration} year{duration > 1 ? "s" : ""}</p>
        <input
          type="range"
          min={1}
          max={10}
    ...
    {mode === "lumpsum" && (
      <div>
        <p className="font-semibold text-lg mb-2">₹{lumpAmt.toLocaleString()}</p>
        <input
          type="range"
          min={fund.minLumpsum}
          max={200000}
        ...
        <div className="mt-4 border-t pt-4 text-sm">
          ...
          <p className="font-semibold mt-2">
            Would have become{" "}
            <span className="text-green-600">
              ₹{lumpFV_selected.toLocaleString()} ({lumpGainPct_selected.toFixed(2)}%)
            </span>
          </p>
  ...
  <div className="bg-white shadow-lg rounded-2xl p-6 flex-1 h-80">
    <h2 className="text-2xl font-semibold mb-6">Minimum Investment Amounts</h2>
    <div className="flex flex-col gap-4">
      ...
      <div className="flex justify-between border-b py-2">
        <span className="text-gray-600">Min. for 2nd investment onwards</span>
...
<div className="lg:flex gap-12 items-start mt-10 hidden">
  <div className="space-y-3 w-1/2">
    <h2 className="text-lg font-semibold mb-3">Equity Sector Allocation</h2>
    {data.map((item, index) => {
...
{/* Returns & Rankings */}
<div className="bg-white shadow-lg rounded-2xl p-6 max-w-4xl mt-10 overflow-x-auto">
  <h2 className="text-2xl font-semibold mb-4">Returns & Rankings</h2>
  <table className="min-w-full table-auto border-collapse text-sm sm:text-base">
    <thead>
      <tr className="text-left border-b">
        <th className="py-3 px-2 sm:px-4 font-medium text-gray-600" colSpan={1}>Category: Commodities Gold</th>
...
<div className="bg-white shadow-lg rounded-2xl p-6 max-w-3xl mt-10 space-y-3">
  <h2 className="text-2xl font-semibold">Expense Ratio, Exit Load & Tax</h2>
  <p className="font-semibold">Expense ratio: 0.10%</p>
  <p className="text-gray-500 text-sm">Inclusive of GST</p>
  <p className="font-semibold mt-2">Exit load</p>
  <p className="text-gray-500 text-sm">Exit load of 1% if redeemed within 15 days.</p>
  <p className="font-semibold mt-2">Stamp duty</p>
  <p className="text-gray-500 text-sm">0.005% (from July 1st, 2020)</p>
  <p className="font-semibold mt-2">Tax implication</p>
